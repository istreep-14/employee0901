 ...
 * Bar Employee CRM - Google Apps Script Backend (Updated for HTML Integration)
 * 
 * Setup Instructions:
...
function doGet(e) {
  try {
    ...
    // Return error page if main HTML fails
...
function getCRMSheet() {
  try {
    ...
    // Create new sheet if none exists or is inaccessible
    if (!sheet) {
...
function initializePositionsSheet(spreadsheet) {
  try {
    ...
    // Set column widths
    posSheet.setColumnWidth(1, 200); // Position Name
    posSheet.setColumnWidth(2, 80);  // Icon
...
function getSystemInfo() {
  try {
    ...
    // Get or create sheet if needed
    if (!sheetAccessible) {
      try {
        getCRMSheet(); // This will create if needed
...
function getAllEmployees() {
  try {
    ...
    values.forEach((row, index) => {
      try {
        // Skip completely empty rows or rows without Employee ID
        ...
        try {
          if (row[9]) {
            const createdDate = row[9] instanceof Date ? row[9] : new Date(row[9]);
            employee.createdDate = !isNaN(createdDate.getTime()) ? createdDate.toISOString() : '';
          }
        } catch (e) {
          Logger.log(`Date parsing error for created date in row ${index + DATA_START_ROW}: ${e.toString()}`);
        }
        
        try {
          if (row[10]) {
            const modifiedDate = row[10] instanceof Date ? row[10] : new Date(row[10]);
            employee.lastModified = !isNaN(modifiedDate.getTime()) ? modifiedDate.toISOString() : '';
          }
        } catch (e) {
          Logger.log(`Date parsing error for modified date in row ${index + DATA_START_ROW}: ${e.toString()}`);
        }
...
/**
 * Save all employees to the sheet with enhanced validation
 */
function saveAllEmployees(employees) {
  try {
    ...
    employees.forEach((emp, index) => {
      try {
        ...
        const cleanEmployee = {
          ...
          createdDate: emp.createdDate || now.toISOString(),
    ...
    const message = `Successfully saved ${validEmployees.length} employees to Google Sheets`;
    if (errors.length > 0) {
...
/**
 * Get positions list with icons (enhanced for HTML integration)
 */
function getPositionsList() {
  try {
    ...
    // If no valid positions found, return defaults
    ...
    if (positions && positions.length > 0) {
      const positionData = positions.map(pos => {
        if (typeof pos === 'string') {
          return [pos, '📋']; // Default icon for string-only positions
...
function testSetup() {
  try {
    ...
    // Test positions
    const positions = getPositionsList();
    Logger.log('✓ Positions loaded: ' + positions.positions.length);
    
    // Test system info
    const sysInfo = getSystemInfo();
    Logger.log('✓ System info retrieved');
